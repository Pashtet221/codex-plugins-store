...
3. Скопируйте `config/.env.example` в локальный `.env` либо задайте переменные окружения.
...
Пароль приложения создаётся в WordPress: **Пользователи → Профиль → Пароли приложений**. Никогда не коммитьте настоящий пароль в GitHub.

На сайте должны быть установлены основной **Codex Bridge** и, при необходимости, дополнения из `wordpress-plugin/`:

- `codex-bridge-rank-math` — безопасное чтение и изменение разрешённых SEO-полей Rank Math;
- `codex-bridge-service-access` — добавляет тип записи `service` в whitelist Bridge.
